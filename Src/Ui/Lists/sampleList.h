...
class SampleList : public UiList {
...
public:
	...
	void edit(int item, int inc, bool shifted) override {
		...
		case PAN:
			...
			break;
		case START:
			sample.set_start(sample.start() + (inc * (shifted ? kSecond : kMillisecond)));
			break;
		case END:
			sample.set_end(sample.end() + (inc * (shifted ? kSecond : kMillisecond)));
			break;
		case LOOP:
			sample.set_loop(inc > 0);
			break;
		case LOOP_START:
			sample.set_loop_start(sample.loop_start() + (inc * (shifted ? kSecond : kMillisecond)));
			break;
		case LOOP_END:
			sample.set_loop_end(sample.loop_end() + (inc * (shifted ? kSecond : kMillisecond)));
			break;
		case U_TURN:
...
private:
	...
	static const uint32_t kSecond = SAMPLE_RATE;
