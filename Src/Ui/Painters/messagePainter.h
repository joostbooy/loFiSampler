...
public:
	...
	static void draw() {
		if (is_visible) {
			...
			ui_->canvas().fill(x + 4, y + 4, w, h, Canvas::SUBTRACTED);
			ui_->canvas().box(x, y, w, h, Canvas::WHITE, Canvas::BLACK);
			ui_->canvas().frame(x + 4, y + 4, w - 8, h - 8, Canvas::WHITE);
			ui_->canvas().draw_text(x, y, w, h, str.read(), Canvas::CENTER, Canvas::CENTER, Canvas::WHITE);

			if (is_error) {
				ui_->canvas().draw_bitmap(x, y, w, h, Bitmap::WARNING_ICON, Canvas::CENTER, Canvas::BOTTOM);
			}
		}
	}
