...
public:
	...
	static void fill(Window &w, Canvas::Color frame_clr, Canvas::Color fill_clr) {
		canvas_->box(w.x, w.y, w.width, w.height, frame_clr, fill_clr);
	}

	static void shadow(Window &w, int offset) {
		canvas_->fill(w.x - offset, w.y - offset, w.width + (offset * 2), w.height + (offset * 2), Canvas::SUBTRACTED);
	}

	static void text(Window &w, const char* text, Canvas::Allign x, Canvas::Allign y, Canvas::Color color = Canvas::BLACK) {
		canvas_->draw_text(w.x, w.y, w.width, w.height, text, x, y, color);
	}

	static void bitmap(Window &w, Bitmap::Id id, Canvas::Allign x, Canvas::Allign y) {
		canvas_->draw_bitmap(w.x, w.y, w.width, w.height, id, x, y);
	}
	...
	static void draw_footer(const char* const* text, int num_options) {
		...
		const int h_coll = 8;
		const int w_coll = canvas_->width() / num_colls;
		const int y = canvas_->height() - h_coll;

		canvas_->set_font(Font::SMALL);
		canvas_->fill(0, y, canvas_->width(), h_coll, Canvas::GRAY);

		for (int i = 0; i < num_colls; ++i) {
			int x = i * w_coll;

			canvas_->fill(x, y, w_coll - 1, h_coll, Canvas::LIGHT_GRAY);
			canvas_->horizontal_line(x + 1, y - 1, w_coll - 3, Canvas::GRAY);

			if (i < num_options) {
				canvas_->draw_text(x, y - 1, w_coll, h_coll, text[i], Canvas::CENTER, Canvas::CENTER);
			}
		}
	}
...
private:
	...
	static void draw_vertical_scrollbar(int x, int y, int w, int h, int rows_total, int rows_visible, int curr_top_row) {
		...
		canvas_->box(x, y, w, h, Canvas::BLACK, Canvas::WHITE);
		canvas_->box(x + 2, (y + bar_y) + 2, w - 4, SettingsUtils::clip_min(1, bar_height - 4), Canvas::BLACK, Canvas::BLACK);
	}
	...
	static void draw_horizontal_scrollbar(int x, int y, int w, int h, int colls_total, int colls_visible, int curr_left_coll) {
		...
		canvas_->box(x, y, w, h, Canvas::BLACK, Canvas::WHITE);
		canvas_->box((x + bar_x) + 2, y + 2, SettingsUtils::clip_min(1, bar_width - 4), h - 4, Canvas::BLACK, Canvas::BLACK);
	}
